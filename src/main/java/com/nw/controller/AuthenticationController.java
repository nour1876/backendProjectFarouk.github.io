...
public class AuthenticationController {
    ...
    @PostMapping(value = "/signup_candidate/{code}", consumes = MediaType.ALL_VALUE)
    public ResponseEntity<?> signUpCandidate(@Valid @ModelAttribute SignupCandidateRequest signupCandidateRequest, @PathVariable("code") String code) {
        try {
            Optional<CandidateEntity> candidate = candidateRepository.findByCode(code);

            if (candidate.isPresent()) {
                authenticationService.signUpCandidateCode(signupCandidateRequest);

                candidateRepository.save(candidate.get());
            }

            if (candidate.isEmpty()) {
                authenticationService.signUpCandidate(signupCandidateRequest);
            }
